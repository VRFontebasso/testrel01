...
// Actualizamos la función pagarClicked para enviar a WhatsApp
function pagarClicked() {
    var itemsCarrito = document.getElementsByClassName('carrito-items')[0].getElementsByClassName('carrito-item');
    var mensaje = "Olá, gostaria de comprar os produtos:%0A";
    var total = 0;
    ...
    for (var i = 0; i < itemsCarrito.length; i++) {
        ...
        var precioProducto = item.getElementsByClassName('carrito-item-precio')[0].innerText;
        var cantidadProducto = item.getElementsByClassName('carrito-item-cantidad')[0].value;
        var precio = parseFloat(precioProducto.replace('R$', '').replace('.', '').replace(',', '.')) * parseInt(cantidadProducto);
        total += precio;
        mensaje += `%0A${encodeURIComponent(tituloProducto)} x ${cantidadProducto} = R$${precio.toFixed(2).replace('.', ',')}`;
    }

    mensaje += `%0ATotal a pagar: R$${total.toFixed(2).replace('.', ',')}`;
    var telefono = "+5511961887553";
    ...
    while (itemsCarrito.length > 0) {
        itemsCarrito[0].parentNode.removeChild(itemsCarrito[0]);
    }
...
function agregarItemAlCarrito(titulo, precio, imagenSrc){
    ...
    for(var i=0; i < nombresItemsCarrito.length; i++){
        if(nombresItemsCarrito[i].innerText == titulo){
            alert("Produto já consta no carrinho de compras");
            return;
        }
    }
...
function actualizarTotalCarrito(){
    ...
    for(var i=0; i < carritoItems.length; i++){
        ...
        //Alteramos de '$' para 'R$' para ser compatível com o replace abaixo
        var precio = parseFloat(precioElemento.innerText.replace('R$', '').replace('.', '').replace(',', '.'));
        var quantidadeItem = item.getElementsByClassName('carrito-item-cantidad')[0].value;
        var quantidade = parseInt(quantidadeItem);
        console.log(precio);
        total += (precio * quantidade);
    }
